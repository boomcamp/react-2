...
Even though I created a new variable, `obj2` and changed the `name` property on
...
There is a down side to `Object.assign` in this scenario however, which is why
...
Now that `App.js` has access to these components, we can then `render` them.
...
Now each item in the `this.props.employee` array will be referenced in our
...
We'll also want to add an `onClick` attribute that uses an arrow function that
...
And finally we want the text of the `<li>` element to be the name of the
`employee`.
